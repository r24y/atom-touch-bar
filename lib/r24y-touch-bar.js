...
export default {
  ...
  showTouchBar(editor = this.activeEditor) {
    ...
    if (editor instanceof TextEditor) {
      ...
      const fold = new TouchBarButton({
        icon: this.icons.fold,
        click: () => atom.commands.dispatch(editor.getElement(), 'editor:fold-current-row'),
      });

      const unfold = new TouchBarButton({
        icon: this.icons.unfold,
        click: () => atom.commands.dispatch(editor.getElement(), 'editor:unfold-current-row'),
      });
      ...
      controls = [...controls, fold, unfold, bracketMatch, new TouchBarSpacer({size: 'flexible'}), bookmarkSegment];
    }
